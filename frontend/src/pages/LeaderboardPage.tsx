...
export default function LeaderboardPage() {
  ...
  if (loading) {
    return <div className="max-w-[720px] mx-auto px-8 py-10 text-ink-muted">Loading leaderboard...</div>
  }

  if (error) {
    return <div className="max-w-[720px] mx-auto px-8 py-10 text-alert-red">{error}</div>
  }
  ...
  return (
    <div className="max-w-[720px] mx-auto px-8 py-10 pb-24">
      ...
      {podium.length > 0 && (
        <div className="flex justify-center items-end gap-3.5 mb-8">
          {podium.map((p, i) => (
            <div key={p.id} className="flex flex-col items-center gap-2 w-[120px]">
              <div className={`w-13 h-13 rounded-2xl ${AVATAR_COLORS[i % AVATAR_COLORS.length]} text-white flex items-center justify-center font-heading font-extrabold text-lg`}>
                {initials(p.username)}
              </div>
              <div className="font-heading font-bold text-[13px] text-ink text-center">{p.username}</div>
              <div className="text-[11.5px] text-ink-light">{p.xp} XP</div>
              <div
                className={`w-full rounded-t-[10px] flex items-start justify-center pt-2 ${podiumColors[i]}`}
                style={{ height: `${podiumHeights[i]}px` }}
              >
                <span className="font-heading font-extrabold text-base text-white">{p.rank}</span>
              </div>
            </div>
          ))}
        </div>
