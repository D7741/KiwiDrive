...
export default function QuestionCard({
  ...
  return (
    <div className="w-full max-w-[560px] bg-card rounded-3xl p-7 shadow-[0_2px_0_var(--color-border-subtle),0_12px_28px_oklch(30%_0.03_260/0.08)] box-border">
      <div className="font-heading font-bold text-xl text-ink mb-5 leading-snug">
        {question}
      </div>
      <div className="flex flex-col gap-3">
        {options.map((opt, i) => (
          <button
            ...
            className={`text-left font-semibold text-[15px] px-4.5 py-4 rounded-2xl w-full box-border transition-colors ${getOptionClass(i)}`}
          >
            {opt.text}
          </button>
        ))}
      ...
      {revealed && explanation && (
        <div className="mt-4 px-4 py-3.5 rounded-xl bg-cream text-sm text-ink-muted leading-relaxed">
